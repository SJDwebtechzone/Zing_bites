...
const brainDir = 'C:\\Users\\KRISHNAKUMAR P\\.gemini\\antigravity\\brain\\c332a7c0-78c5-4a8a-848d-2e77fdab6bfe';
...
images.forEach(img => {
  const src = path.join(brainDir, img.src);
  const dst = path.join(targetDir, img.dst);
  if (fs.existsSync(src)) {
    fs.copyFileSync(src, dst);
    console.log(`✅ Copied: ${img.dst}`);
  } else {
